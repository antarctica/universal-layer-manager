...
export type LayerManagerActor<TLayer, TGroup = TLayer> = ActorRefFrom<ReturnType<typeof createLayerManagerMachine<TLayer, TGroup>>>;
...
export function createLayerManagerMachine<TLayer, TGroup = TLayer>() {
  return setup({
    ...
    actions: {
      'Add new layer': enqueueActions(({ enqueue, context, self }, params: AddLayerParams<TLayer, TGroup>) => {
    ...
    guards: {
      isValidLayerConfig: ({ context }, params: AddLayerParams<TLayer, TGroup>) => isValidLayerConfig(params.layerConfig, context),
    },
  }).createMachine({
    id: 'layerManager',
    context: ({ input }) => ({
